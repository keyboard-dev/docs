...
// The pre-docs.dev Mintlify site served pages at the repo root
// (docs.keyboard.dev/getting-started/quickstart); this site serves them
// under /docs. Redirect the old paths so inbound links keep working.
const LEGACY_PATHS = {
  '/getting-started/index': '/docs',
  '/getting-started': '/docs',
  '/guides/app_builder': '/docs/guides/app-builder',
};
const LEGACY_PREFIX = /^\/(getting-started|guides|how-it-works)\//;
  ...
  async fetch(request, env, ctx) {
    {
      ...
      const target =
        LEGACY_PATHS[url.pathname] ??
        (LEGACY_PREFIX.test(url.pathname) ? `/docs${url.pathname}` : null);
      if (target) {
        url.pathname = target;
